...
export default function YouTubeStarterKitUtility() {
  ...
  const output = useMemo(() => {
    ...
    const viewerRetention = `${48 + (seed % 38)}%`;
...
The channels winning in 2026 are not
the biggest.
...
`;
  ...
  const downloadBlueprint = () => {
    ...
    link.download = `${safeTopic}-creator-blueprint.txt`;
    ...
    alert("Creator blueprint downloaded successfully.");
  };
        ...
        <Card className="border-white/10 bg-white/[0.04] text-white shadow-2xl">
          <CardContent className="p-6">
            ...
            <button
              ...
              className={`mt-5 w-full rounded-2xl border border-amber-300/20 px-4 py-3 font-semibold transition ${isGenerating
                ? "bg-amber-300/5 text-amber-100/40 cursor-not-allowed"
            ...
            <div className="mt-2 text-center text-xs text-neutral-500">
              {isGenerating
                ? "Creator engine is recalculating angles, hooks, and monetization signals..."
                : "Click to generate a fresh strategic variation."}
            </div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <Button
                onClick={copyAll}
                className="w-full rounded-2xl bg-amber-300 py-4 text-base font-semibold text-neutral-950 hover:bg-amber-200"
              >
                {copied ? (
              ...
              <Button
                ...
                className="w-full rounded-2xl border border-white/10 bg-white/[0.04] py-4 text-base font-semibold text-white hover:bg-white/[0.08]"
              >
                Download blueprint
              </Button>
            </div>
              ...
              <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <h3 className="mt-3 text-3xl font-bold">
                    Unlock the full creator operating system
                  </h3>

                  <p className="mt-4 max-w-2xl text-neutral-400 leading-8">
                    Advanced monetization maps, viral psychology systems,
                    creator positioning frameworks, trend hijack engines,
                    thumbnail manipulation formulas, and elite faceless
                    channel blueprints.
                  </p>
                ...
                <div className="rounded-3xl border border-white/10 bg-black/40 p-8 text-center shadow-xl">
                  ...
                  <div className="mt-4 text-5xl font-black text-amber-300">
                    $19
                  </div>

                  <div className="mt-2 text-neutral-400">
                    one-time creator upgrade
                  </div>

                  <button
                    onClick={() => {
                      if (!email.includes("@")) {
                        alert("Enter your email first to unlock premium.");
                        return;
                      }

                      setPremiumUnlocked(true);
                      alert("Premium preview unlocked. Payment integration coming next.");
                    }}
